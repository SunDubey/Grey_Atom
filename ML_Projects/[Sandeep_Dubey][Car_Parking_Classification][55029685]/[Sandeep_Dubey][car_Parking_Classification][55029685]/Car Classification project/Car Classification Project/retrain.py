...
def main():
    ...
    if not checkIfNecessaryPathsAndFilesExist():
        return
    # prepare necessary folders that can be used during training
    prepare_file_system()
    ...
    if class_count == 0:
        tf.logging.error('No valid folder named ' + TRAINING_IMAGES_DIR)
        return -1
    if class_count == 1:
    ...
    with tf.Session(graph=graph) as sess:
        ...
        # if completed all training, run a final test evaluation on some new images we haven't used before
...
def checkIfNecessaryPathsAndFilesExist():
    ...
    # if any training sub-folders has less than the recommended number of training images, show a warning
    for trainingSubDir in trainingSubDirs:
    ...
    # if the test images directory does not exist, show and error message
    ...
    # if there are not enough images in the test images directory, shows an error and return false
...
def prepare_file_system():
    # Setup the directory and write summaries to for TensorBoard
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    Analyzes the sub folders in the image directory, splits them into stable training, testing, and validation sets, and returns a data structure describing the lists of images for 
    each label and their paths.
    ...
    # if the image directory does not exist,rause an error
    ...
    for sub_dir in sub_dirs:
        ...
        # if the file list is empty at this point, give a warning
        if not file_list:
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir, category, bottleneck_dir, jpeg_data_tensor,
    ...
    # read the contents of the bottleneck file as one big string
    ...
    if errorOccurred:
        ...
        # read the contents of the newly created bottleneck file
        with open(bottleneck_path, 'r') as bottleneck_file:
            bottleneckBigString = bottleneck_file.read()
